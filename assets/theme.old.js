...
theme.Slideshow = (function() {
  ...
  slideshow.prototype.setSlideshowHeight = function() {
    ...
    if(this.$slideshow.hasClass('_slideshow') && $(window).width() < 768) {} else { //CD
...
theme.Cart = (function() {
  var selectors = {
    ...
    jsChkGift: '.jsChkGift', //CD
    cartItemHtml: '#cartItemHtml' //CD
  };
  ...
  function Cart(container) {
    ...
    //CD
    this._checkToRemoveGift(this);
    this.$container.on('change',selectors.jsChkGift,$.debounce(500, this._manageGift.bind(this)));
  ...
  Cart.prototype = _.assignIn({}, Cart.prototype, {
    _setupCartTemplates: function() {
      this.$itemTemplate = $($(selectors.cartItemHtml, this.$container).html()); //CD*
    ...
    //CD
    _checkToRemoveGift: function(_this){
      if(_this.$container.find('.jsGiftWrap').attr('data-action') == 'remove') {
        var $chk = _this.$container.find(selectors.jsChkGift);
        var params = {
          url: '/cart/change.js',
          data: { quantity: 0, id: $chk.prop('value') },
          dataType: 'json'
        }
        $.post(params)
        .done(function(state) {
    ...
    _manageGift: function(evt){
      var $chk = $(evt.target);
      var _id = null;
      if($('[data-cart-item][data-cart-item-id="'+$chk.prop('value')+'"]')) _id = $('[data-cart-item][data-cart-item-id="'+$chk.prop('value')+'"]').attr('data-cart-item-index');
    ...
    _createCart: function(state) {
      ...
      //CD
      if($('.jsGiftWrap').length && $('.jsGiftProId').length) {
        if(state.item_count > 0) {
          var giftFound = false,cartCount = 0,gift_by_product;
          for(var i = 0;i < state.items.length;i++) {
            if(typeof state.items[i] == 'undefined') continue;
            if(state.items[i].product_id == $('.jsGiftProId').val()) {
              gift_by_product = false;
              if(state.items[i].properties != null) {
                if(typeof state.items[i].properties['_added_with_product'] != 'undefined') gift_by_product = true;
              }
              if(!gift_by_product) giftFound = true;
            } else {
              cartCount++;
            }
          }
          if(giftFound) {
            $('.jsGiftWrap').addClass('hide');
            if(cartCount == 0) {
              this.$container.find('.jsGiftWrap').attr('data-action','remove');
              this._checkToRemoveGift(this);
            }
          } else {
            $('.jsGiftWrap').removeClass('hide');
          }
        } else {
          $('.jsGiftWrap').addClass('hide');
        }
    ...
    _createLineItemList: function(state) {
      return $.map(
        ...
        function(item, index) {
          ...
          //CD
          if($('.jsGiftProId').length) {
            if($('.jsGiftProId').val() == item.product_id) {
              $(selectors.cartItemImage, $item).addClass('itemImg').removeClass('cart__image');
              $(selectors.cartItemTitle, $item).attr('href','javascript:void(0);');
              if(item.properties != null) {
                if(typeof item.properties['_added_with_product'] != 'undefined') {
                  return '<input type="hidden" class="gift_item" data-id="'+item.properties['_added_with_product']+'" data-cart-item-index="'+parseInt(index+1)+'" data-cart-item-quantity="'+item.quantity+'">';
                }
              }
            }
          }
          ...
          $(selectors.cartRemove, $item).attr('data-id',item.product_id);
    ...
    _setQuantityInputs: function($item, item, index) {
      ...
      if($('.jsGiftProId').length) {
        if($('.jsGiftProId').val() == item.product_id) {
          $('.cart__qty', $item).find('.cart__qty-input').remove();
          $('.cart__qty', $item).append('<p class="itemQty">'+item.quantity+'</p>');
        }
      } 
    },
    ...
    _onRemoveItem: function(evt) {
      ...
      var $gift = $('.gift_item[data-id="'+$remove.attr('data-id')+'"]');
      if($gift.length) {
        var updates = [];
        $('[data-cart-item-quantity]').each(function(){
          if(parseInt($(this).attr('data-cart-item-index')) == parseInt($lineItem.attr('data-cart-item-index')) || parseInt($(this).attr('data-cart-item-index')) == parseInt($gift.attr('data-cart-item-index'))) {
            updates.push(0);
          } else {
            updates.push(parseInt($(this).attr('data-cart-item-quantity')));
          }
        });
        params = {
          url: '/cart/update.js',
      ...
      // cd code
      var main_item_old_qty = parseInt($($remove).parents('.cart__row').attr('data-cart-item-quantity'));
        ...
        .done(
          function(state) {
            if (state.item_count === 0) {
              ...
              // cd code
              var _this = this;
              $($remove).parents('.cart__row').find('.product-details .product-details__item-label[data-cart-item-property-name]').each(function( index ) {               
                if($(this).text() == ('Personalized DOB: ') || $(this).text() == ('Personalized DOB')){
                  if($('.cart__row[data-cart-item-key*="40131415048271"]').length){
                    var extra_item_qty = parseInt($('.cart__row[data-cart-item-key*="40131415048271"] .cart__quantity-td .cart__qty .cart__qty-input').val());
                    var extra_new_qty = extra_item_qty - main_item_old_qty;
                    $.ajax({
                      url: '/cart/change.js',
                      type: 'post',
                      dataType: 'json',
                      ...
                      success:function(items) {
                        if (items.item_count === 0) {                          
                          ...
                          _this._createCart(items);
                          _this._showRemoveMessage($lineItem.clone());
                          $('#CartCount span[data-cart-count]').text(items.item_count);
                        }
                      },
...
theme.Product = (function() {
  function Product(container) {
    ...
    this.selectors = {
      ...
      preOrderProduct: '[cust-pre-order]'
    };
    ...
    this.$preOrderProduct = $(this.selectors.preOrderProduct);
...
//   CD           self._initThumbnailSlider();
    ...
    _initAddToCart: function() {
      $(this.selectors.productForm, this.$container).on(
        ...
        function(evt) {
          ...
          // CD CODE          
          if($('.product-form__controls-group .selector-wrapper').length){
          	var select_valid = $('.product-form__controls-group .selector-wrapper .single-option-selector option:selected').val();
            if(select_valid == ''){
              $('.select_error').addClass('show');
              return false;
            }else{
              $('.select_error').removeClass('show');
            }
          }

          // cd code
          if($('.custom_personalized .custom_personalized_tab .custom_personalized_title.personalized_show.tab_open').length && $('.custom_personalized .custom_personalized_block .personalized_field_block .text_color_block input').length){
            if(!$('.custom_personalized .custom_personalized_block .personalized_field_block .text_color_block input:checked').length){
              $('.personalized_error_msg').text('Please Select Text Color');
              $('.personalized_error_msg').removeClass('hide');
              return false;
            }else if($('.custom_personalized .custom_personalized_block .personalized_field_block input[name="properties[Name]"]').val() == ''){
              $('.personalized_error_msg').text('Please Enter Name');
              $('.personalized_error_msg').removeClass('hide');
              return false;
            }
          }
          
          if($('#cust_personalized_block:visible').length){
            if($('.custom_personalized .custom_personalized_block .personalized_field_block input[name="properties[Name]"]').val() == ''){
              $('.personalized_error_msg').text('Please Enter Name');
              $('.personalized_error_msg').removeClass('hide');
              return false;
            }else{
              $('.personalized_error_msg').addClass('hide');
            }
          }
          ...
          var $gift = $form.find('.chk_add_gift'),value;
          var gift_checked = false;

        
          var $christmas_gift = $form.find('.christmas-chk_add_gift'),christmas_value;
          var christmas_gift_checked = false;


           $form.find('[name="properties[Christmas Add-On]"]').remove();
          ...
          setTimeout(function(e){
            console.log('working');
            if($christmas_gift.length) {
            christmas_value = $christmas_gift.prop('value').trim();
            if($christmas_gift.is(':checked') && christmas_value != '') {
               evt.preventDefault();
            ...
            if(!$christmas_gift) {
              if($form.find('.christmas_add_gift').length) $form.find('.christmas_add_gift').remove();
            }
          },500)
            ...
            if($gift.is(':checked') && value != '') {
              ...
              $.ajax({
                ...
                complete: function(data) {
                  console.log(data);
                  if (!_this.ajaxEnabled) {
                    $form[0].submit();
                  } else {
                    _this.$previouslyFocusedElement = $(':focus');

                    var isInvalidQuantity = _this.$quantityInput.val() <= 0;

                    if (isInvalidQuantity) {
                      _this._showErrorMessage(theme.strings.quantityMinimumMessage);
                      return;
                    }

                    if (!isInvalidQuantity && _this.ajaxEnabled) {
                      // disable the addToCart and dynamic checkout button while
                      // request/cart popup is loading and handle loading state
                      _this._handleButtonLoadingState(true);
                      var $data = $form;
                      _this._addItemToCart($data);
                      return;
                    }
                  }
                }
          ...
          if(!gift_checked) {
            if($form.find('.add_gift').length) $form.find('.add_gift').remove();
    ...
    _addItemToCart: function(data) {
      ...
      $.post(params)
        .done(
          function(item) {
            // cd code
            var personalized = $('#custom_dob_field');
            if($(personalized).attr('persobalized-product')){
              $.ajax({
                url: '/cart/add.js',
                type: 'post',
                dataType: 'json',
                data: {
                  ...
                  quantity: 1,
                  properties: {
                    'extra_dob_product': item.product_id
                  }
                },
                success:function(items) {
                },
    ...
    _updateAddToCart: function(evt) {
      ...
      if (variant) {
        if(!$(this.selectors.preOrderProduct).length){
...
// $(document).on('click', '.personalise-checkbox .checkbox', function(){
//   if($(this).is(':checked') == true){
//     $('.Personalise-text_confirm').show();
//     if($('.personalise-checkbox .confirm_checkbox').is(':checked') == true){
//       $('.confirmation .product-form__cart-submit').attr('disabled', false);
//       $('.confirmation .shopify-payment-button__button').attr('disabled', false);
//     }else{
//       $('.confirmation .product-form__cart-submit').attr('disabled', true);
//       $('.confirmation .shopify-payment-button__button').attr('disabled', true);
//     }
//   }else{
//     $('.Personalise-text_confirm').hide();
//     $('.personalise-text').val('');
//     $('.confirmation .product-form__cart-submit').attr('disabled', false);
//     $('.confirmation .shopify-payment-button__button').attr('disabled', false);
//   }
// });
$(document).on('click', '.personalise-checkbox .confirm_checkbox', function(){
  if($(this).is(':checked') == true){
    $('.product-form-product-template .product-form__cart-submit').attr('disabled', false);
    $('.confirmation .shopify-payment-button__button').attr('disabled', false);
  }else{
    $('.product-form-product-template .product-form__cart-submit').attr('disabled', true);
    $('.confirmation .shopify-payment-button__button').attr('disabled', true);
  }
});
$(document).on('click', '.show_checkbox', function(){
  if($(this).is(':checked') == true){
    $('#tab_content1').show();
    $('.check-boxes').show();
    $("#deny_checkbox").removeAttr('checked');

  }
});
$(document).on('click', '.personalise-checkbox .deny_checkbox', function(){
  if($(this).is(':checked') == true){
    $('#tab_content1').hide();
    $(".show_checkbox").removeAttr('checked');
    $('.check-boxes').hide();
    $('.product-form-product-template .product-form__cart-submit').attr('disabled', false);
    $('.confirmation .shopify-payment-button__button').attr('disabled', false);
  }else{
...
$(document).ready(function(){
//   if($('.personalise-checkbox .checkbox').is(':checked') == true){
//     $('.Personalise-text_confirm').show();
//     if($('.personalise-checkbox .confirm_checkbox').is(':checked') == true){
//       $('.confirmation .product-form__cart-submit').attr('disabled', false);
//       $('.confirmation .shopify-payment-button__button').attr('disabled', false);
...
//   }
  ...
  if($('.select-charity .radio').is(':checked')){
    var value = $('.select-charity .radio:checked').parent().attr('data-val');
    $('.select-charity .radio:checked').val(value);
    $('.cart__submit-controls').find('.cart__submit').attr('disabled', false);
  }
});

$(document).on('click', '.js-read-more-text', function(){
  if($('.more_text').hasClass('ourmission-more_text')){
    $('.more_text').slideUp();
    $('.more_text').removeClass('ourmission-more_text');
    $(this).text('read more');
  }else{
    $(this).text('read less');
    $('.more_text').addClass('ourmission-more_text');
    $('.more_text').slideDown();
  }
});
$(document).on('click', '.select-charity .radio', function(){
  var value = $(this).parent().attr('data-val');
  if($(this).is(':checked')){
    $('.cart__submit-controls').find('.cart__submit').attr('disabled', false);
    $('.select-charity .radio').removeAttr('value');
    $(this).val(value);
  }
});
...
function applyFilter() {
  var values = [],url,tmp;
  $('.jsFilterWrap').each(function(){
    if($(this).find('.jsFilter.active').length) {
      values.push($(this).find('.jsFilter.active').attr('data'));
    }
  });
  tmp = location.pathname.substr(1).split('/');
  if(values.length) {
    values = values.join('+');
    url = '/collections/'+tmp[1]+'/'+values;
    console.log("valuesvalues", url)
  } else {
    url = '/collections/'+tmp[1];
  } 
  if(location.search != '') {
...
$('.filter-title').on('click',function(){
  ...
  if($parent.find('.filter-opts').is(':visible')) {
    $parent.removeClass('active');
    $parent.find('.filter-opts').slideUp();
  } else {
    $('.filter-group').removeClass('active');
    $('.filter-group').find('.filter-opts').slideUp();
    $parent.addClass('active');
    $parent.find('.filter-opts').slideDown();
  }
  if($parent.hasClass('menu-group')) {
    $parent.siblings('.filter-group').removeClass('active').find('.filter-opts').slideUp();
  }
});
...
$('.jsFilterClear').on('click',function(e){
  e.preventDefault();
  $(this).parents('.jsFilterWrap').find('.jsFilter').removeClass('active');
  applyFilter();
});
...
$(document).ready(function(){
  ...
  $('.bundleSel').on('change',function(){
    var $name = $(this).parents('.bundleItem').find('.bundleName');
    $name.val($name.attr('data-title')+' - '+$(this).val());
  });
  
  $('.colorLabel').on('click',function(){
    var $name = $(this).parents('.bundleItem').find('.bundleName');
    $name.val($name.attr('data-title')+' - '+$(this).attr('data'));
    $(this).addClass('selected').parents('.colorOpt').siblings('.colorOpt').find('.colorLabel').removeClass('selected');
  });
});
...
$('.js-mobile_sort_by_filter').on('click',function(){
  $(this).find("#sortBy-filterSpan").toggleClass("open");
  $(this).find("#SortBy").toggleClass("open");
  $(this).find("#SortBy").toggleClass("hidden");
});


$(".tab-nav-link:first").addClass("current");
var first_tab = $(".tab-nav-link:first").attr("data-id");
$("#"+first_tab+"").removeClass("hide");

$(".tab-nav-link").on("click",function(){
  	$(".tab").addClass("hide");
	$(".tab-nav-link").removeClass("current");
...
$(document).on("click",".accordion",function(){
  if($(this).hasClass("change_icon")){
   ...
   $(this).parent().find(".panel").slideUp();
    ...
    $(".accordion").removeClass("change_icon");
    $(".accordion").parent().find(".panel").slideUp();
    $(this).addClass("change_icon");
  	$(this).parent().find(".panel").slideDown();
  }
});

$(document).on("click",".cart__footer .info_text",function(){
    $(".cart__footer .info_show_text").slideToggle();
});

// CD CODE
if ($(window).width() < 768) {
$('.announcement-list-crousal').slick({
  slidesToShow: 1,
  slidesToScroll: 1,
  autoplay: false,
  dots: false,
  ...
  adaptiveHeight: true,
  asNavFor: '.thumbnails-wrapper_slider'
});
...
$( window ).on( "load", function(){
	$('.announcement_bar-section').css('visibility', 'visible');
});

$(document).on('click', '.qtybox .btnqty', function(){
  var qty = parseInt($(this).parent('.qtybox').find('.quantity-input').val());
   var tagsplit_val = parseInt($('.tagsplit_val').attr('value'));
  if($(this).hasClass('qtyplus')) {
    // if($('.template-product').length && $('.tagsplit_val').length){
    //   qty = qty + tagsplit_val;
    // }else{
      qty++;
    // }
  }else {
...
$(document).on("click",".personalise_box-tab .tab_buttons .tab_btn",function(){
  if(!$(this).hasClass('active_tab')){
    var tab_id = $(this).attr('data-id');
    $('.personalise_box-tab .tab_buttons .tab_btn').removeClass('active_tab');
    $('.personalise_box-tab .tab_content .tab_list').slideUp();
    $(this).addClass('active_tab');
    $('#' + tab_id).slideDown();
  }

  if(tab_id == 'tab_content1'){
    if($('.personalise-checkbox .confirm_checkbox').is(':checked') == true){
      $('.product-form-product-template .product-form__cart-submit').attr('disabled', false);
      $('.confirmation .shopify-payment-button__button').attr('disabled', false);
    }else{
...
// cd code
if ($(window).width() < 768) {
  if($(".image_grid_slider_mobile").length){
    $(".image_grid_slider_mobile").each(function( index ){
      var e = $(this);
      $(".image_grid_slider_mobile").slick({
        slidesToShow: 1,
        slidesToScroll: 1,
        infinite: false,
...
$(document).on('click', '.custom_personalized .custom_personalized_tab .custom_personalized_title', function(){
  $('.custom_personalized .custom_personalized_tab .custom_personalized_title').removeClass('tab_open');
  $(this).addClass('tab_open');
  if($(this).hasClass('personalized_show')){
    $('.custom_personalized .custom_personalized_content #cust_personalized_block').slideDown();
    $(".pro_confirm").css("display", "flex");
    // CD code
    $("#ProductSection-product-template .grid .grid__item .product-form #tab_content2 button.product-form__cart-submit").attr("disabled");
    $("#ProductSection-product-template .grid .grid__item .product-form #tab_content2 button.product-form__cart-submit").addClass("disabled");
  }else{
    $('.custom_personalized .custom_personalized_content #cust_personalized_block').css("display", "none");
    $(".pro_confirm").css("display", "none");
    $("#ProductSection-product-template .grid .grid__item .product-form #tab_content2 button.product-form__cart-submit").removeAttr("disabled");
    $("#ProductSection-product-template .grid .grid__item .product-form #tab_content2 button.product-form__cart-submit").removeClass("disabled");
  }
  if($(this).hasClass('dont_persolnalized')){
    $('.personalized_error_msg').addClass('hide');
    $('.custom_personalized .custom_personalized_block .personalized_field_block .text_color_block input[type="radio"]').removeAttr('checked');
    $('.custom_personalized .custom_personalized_block .personalized_field_block input[type=text]').val('');
    $('.custom_personalized .custom_personalized_block .personalized_field_block input[type=text]').css({'color': 'grey', 'background-color': 'transparent'});
    $('.custom_personalized .custom_personalized_content .custom_personalized_block .personalized_field_block input#custom_dob_field').removeAttr('persobalized-product');
  }
});

$(document).on('click', '.custom_personalized .custom_personalized_block .personalized_field_block .text_color_block label', function(){
  var color_value = $(this).attr('color-value');
  $('.custom_personalized .custom_personalized_block .personalized_field_block input[type=text]').css({'color': ''+ color_value +'', 'background-color': 'transparent'});
  if(color_value == 'white'){
    $('.custom_personalized .custom_personalized_block .personalized_field_block input[type=text]').css({'color': 'grey', 'background-color': 'transparent'});
  }
});

$(document).on('change', '.custom_personalized .custom_personalized_content .custom_personalized_block .personalized_field_block input#custom_dob_field', function(){
  $(this).attr('persobalized-product', 'true');
});

$(document).on('change', '.cart__row .cart__qty .cart__qty-input', function(){
  var main_item_old_qty = parseInt($(this).parents('.cart__row').attr('data-cart-item-quantity'));
  var main_item_new_qty = parseInt($(this).val());
  $(this).parents('.cart__row').find('.product-details .product-details__item-label[data-cart-item-property-name]').each(function( index ) {
    if($(this).text() == ('Personalized DOB: ') || $(this).text() == ('Personalized DOB')){
      if($('.cart__row[data-cart-item-key*="40131415048271"]').length){
        var extra_item_qty = parseInt($('.cart__row[data-cart-item-key*="40131415048271"] .cart__quantity-td .cart__qty .cart__qty-input').val());
        if(main_item_old_qty > main_item_new_qty){
          var extra_new_qty = extra_item_qty - 1;
        }else{
          var extra_new_qty = extra_item_qty + 1;
        }
        extra_pro_update(extra_new_qty);
      }
    }
  });
});

function extra_pro_update(extra_new_qty) {
  $.ajax({
    url: '/cart/change.js',
    type: 'post',
    dataType: 'json',
...
// $(document).ready(function(){
//   $('.template-product .product-form__controls-group .single-option-selector').prepend('<option value="" disabled selected>Choose an option</option>');
// })

$(document).on("click",".cust_tab",function(){
  if($(this).hasClass("cust_tab_open")){
    $(this).removeClass("cust_tab_open");
   $(this).next(".cust_panel").slideUp();
  }else{
    $(".cust_tab").removeClass("cust_tab_open");
    $(".cust_tab").next(".cust_panel").slideUp();
    $(this).addClass("cust_tab_open");
  	$(this).next(".cust_panel").slideDown();
  }
});
...
$(document).ready(function(){
  var announcment_bar = $('.announcement_bar-change').height();
  $('.header_js').css('top', announcment_bar + 'px');
});

// cd code
$(document).on('click', '.read_more_less', function(){
  if($(this).parents('.custom_collection_header').hasClass('active')){
    $(this).parents('.custom_collection_header').removeClass('active');
    $(this).find('span').html("Read More");
  }else{
    $(this).parents('.custom_collection_header').addClass('active');
    $(this).find('span').html("Read Less");
  }
});

$(window).on('load',function(e) {
 $("#custom_time_field").datetimepicker({
    format: "LT",
    icons: {
      up: "fa fa-chevron-up",
...
// CD shubham
 $(document).ready(function () {
    $(".text_font_block label").click(function () {
      var font_family = $(this).prev(".text_font_block input").attr("value");
      $(".personalized_field_block #custm_name").css("font-family",font_family);
    });
});

//CD Nirav
function cart_ajax($_this){
  $.getJSON("/cart.js",function(state) {
    var main_pro_qty = 0,
        free_pro_qty = 0;
    $(state.items).each(function(index,item) {
      if(item.properties['Date of Birth'] != undefined){
        main_pro_qty = main_pro_qty + item.quantity;
      }else if(item.id == 40131415048271){
        free_pro_qty = free_pro_qty + item.quantity;
      }
    });
    if(main_pro_qty != free_pro_qty){
      $.ajax({
        url: '/cart/change.js',
        type: 'post',
        dataType: 'json',
        ...
        success:function(items) {
          $.getJSON("/cart.js",function(cart) {
            ...
            $_this._setCartCountBubble(cart.item_count);
          });
        },
        error:function(error){
          console.log('Error Found!!!');
...
// CD Sanket
$(document).on('click', '#shopify-section-header nav ul .site-nav--has-dropdown', function(e){
  e.preventDefault();
    $('#shopify-section-header nav ul .site-nav--has-dropdown').removeClass('show_menu');
    $(this).addClass('show_menu');
});

$(window).click(function(e) {
  if(!$(e.target).hasClass('site-nav__label')){
    $('#shopify-section-header nav ul .site-nav--has-dropdown').removeClass('show_menu');
  }
});
...
// }
 if ($(window).width() < 767) {
   $('.thumbnails-wrapper_slider').slick({
    slidesToShow: 3,
    slidesToScroll: 1,
     arrows:false,
    // prevArrow: $('.thumbnails-slider__prev'),
    // nextArrow: $('.thumbnails-slider__next'),
    asNavFor: '.announcement-list-crousal',
    focusOnSelect: true
  });
}

$(document).ready(function() {
  var announheight = $('#shopify-section-announcement-bar').height();
  var headerheight = $('.header_js').height();
  var bothheight = headerheight + announheight;  
  $("#shopify-section-header").css("padding-top", bothheight +"px");

  if($('.var_btn .swatch input').length && $('.var_btn .custom_gift_select_box').length){
    $(document).on('change', '.var_btn .swatch input', function(){
      var current_name = $(this).attr('value'),
          data_option_index = parseInt($(this).parents('.swatch.clearfix').attr('data-option-index'));
      // CD Nirav
      $(this).parents('.varient-group').find('.selector-wrapper.as-'+(data_option_index+1)+' select.single-option-selector').val(current_name).trigger('change');
      
      // if($(this).parents('.selector-wrapper').find('select').hasClass('choose-the-design')){
      //   if($('.var_btn .choose-the-design.custom_gift_select_box').length){
      //     $('.var_btn .choose-the-design.custom_gift_select_box').val(current_name).trigger('change');
      //   }
      // }else{
      //   if($('.var_btn .custom_gift_select_box').length && $('.var_btn .choose-the-design.custom_gift_select_box').length == 0){
      //     var dif_value = $('.var_btn .custom_gift_select_box').val();
      //     $('.var_btn .custom_gift_select_box').val(current_name).trigger('change');
      //   }
      //   if($('.var_btn .choose-the-design.custom_gift_select_box').length && $('.var_btn .size.custom_gift_select_box').length){
      //     $('.var_btn .size.custom_gift_select_box').val(current_name).trigger('change');
          
      //     var dif_value = $('.var_btn .choose-the-design.custom_gift_select_box').val();
      //     $('.var_btn .choose-the-design.custom_gift_select_box').val(dif_value).trigger('change');
      //   }
      // }
    });
  }
...
$(".blog_filter_title").click(function(){
  $(".blog_filter").toggleClass("show_in");
});

$(document).on('click', '.qtyminus', function(){
  var qty_button=$('.product-form__input.quantity-input');
  var qty_tag_val = $('.tagsplit_val').text();
...
$(document).on('click', '.qtyplus', function(){
  $(".qtyminus").removeClass("intro");
});

if($('.review_slider_main').length) {
    if ($(window).width() > 767) {
...
if($('.new_filter_slider').length){
  if ($(window).width() > 768) {
    $('.new_filter_slider').slick({
      slidesToShow: 7,
      slidesToScroll: 1,
      infinite: false,
      prevArrow: $('.fliter_prev_arrow'),
      nextArrow: $('.fliter_next_arrow')
    });
  }
}

if($('.new_filter_wrrap').length){
  if ($(window).width() < 767) {
    $('.new_filter_wrrap').slick({
      slidesToShow: 4,
      slidesToScroll: 1,
      infinite: false,
      prevArrow: $('.fliter_prev_arrow'),
...
// CD Nirav
$(document).on('change','.template-product .personalized_field_block #dob_field_val',function(){
  var new_date_format = new Date($(this).val()),
      new_Date = new_date_format.getDate(),
      new_Month = new_date_format.getMonth() + 1,
      new_FullYear = new_date_format.getFullYear();
  $('.template-product .personalized_field_block #dob_field').val(new_Date + '-' + new_Month + '-' + new_FullYear);
});

// CD Arshit
$(document).on('click', '#MobileNav.mobile-nav .new_js-toggle-submenu.mobile-nav__link', function(event){
...
function checkCookie() {
  var gift_set = getCookie("Gift Sets");
  if (gift_set != "") {
    
    var arr = $.parseJSON(gift_set); //convert to javascript array
    var bar_close_svg = $('.gift_sets_bar .bar_close_icon').html();
    var total_prce = 0;
    
    if(arr.length > 0 && $('.template-collection.signature-gift-set').length){
      $.each(arr,function(key,value){
        total_prce = total_prce + parseInt(value.price)
        var personalise_html = ''
        var size_variant_html = '';
        if(value.personalise_name_check == true){
          if(value.personalise_name != undefined){
            var personalise_name = value.personalise_name;  
          }else{
            var personalise_name = 'Personalise';
          }
          personalise_html = '<div class="personalise"><img src="https://cdn.shopify.com/s/files/1/0289/3240/7375/files/palette.png?v=1695042120"><span class="personalise_text">'+ personalise_name +'</span></div>'
        }

        if(value.size_variant.length > 0){
          size_variant_html += '<div class="custom_size_variant"><select class="size_variant">';
          $.each(value.size_variant,function(keys,values){
            var split_value = values.split(':');
            if(value.product_id == split_value[0]){
              size_variant_html += '<option value="'+ split_value[0] +'" selected>'+ split_value[1] +'</option>'  
            }else{
              size_variant_html += '<option value="'+ split_value[0] +'">'+ split_value[1] +'</option>'  
            }
          });
          size_variant_html += '</select></div>';
        }
        
        $('.gift_sets_bar .main_product_list').append('<div class="main_product_item" data-pro-index="'+ key +'" data-price="'+ value.price +'" data-avilable="'+ value.pro_avilable +'" data-pro-id="'+ value.product_id +'"><div class="personalise_with_close_icon">'+ personalise_html +'<span class="remove_pro_items">'+ bar_close_svg +'</span></div><div class="image"><img src="'+ value.image +'"></div><h4 class="pro_title">'+ value.title +'</h4>'+ size_variant_html +'</div>')
      });
      $('.gift_sets_bar #setTotal').html(theme.Currency.formatMoney(total_prce,theme.moneyFormatWithCurrency));
      var left = $('.gift_sets_bar .main_product_list').width() + 128;
      $('.gift_sets_bar .bar_scroll_pro').scrollLeft(left);
      if($('.gift_sets_bar .main_product_list .main_product_item').length < 2){
        $('.gift_sets_bar .title-with-close_icon .bar_title').text('Please select at least 2 products');
        $('.gift_sets_bar .page_wrapper .action-buttons-wrap button#addAllToCart, .gift_sets_bar .page_wrapper .action-buttons-wrap button#personaliseAll').prop('disabled', true);
      }
      $('.gift_sets_bar').addClass('is_show');
    }
  }
}
...
$(document).ready(function() {
  if($('.gift_sets_bar .select_gift .gift-checkbox:checked').length){
    $('.gift_sets_bar .select_gift .gift-checkbox').prop('checked', false);
  }
  var r = document.querySelector(':root');
  if($('.gift_sets_bar').length) {
    r.style.setProperty('--gift-bar-down', '-'+$('.gift_sets_bar').outerHeight()+'px');
  }
})

// product grid added
$(document).on('click', '.signature-gift-set .collectionBody li a', function(e){
  e.preventDefault();
  $(this).parents('.grid-view-item').find('.circle_check_icon').addClass('is_added');
  if($(this).attr('data-avilable') == 'true'){
    if($('.gift_sets_bar').hasClass('is_show') == false){
      $('.gift_sets_bar').addClass('is_show');
    }
    var current_image_url = $(this).attr('pro_img_url');
    var current_price = $(this).attr('data-price');
    var current_title = $(this).attr('data-title');
    var close_svg = $('.gift_sets_bar .bar_close_icon').html();
    var size_var = '';
    var personalise_name = $(this).attr('data-personalized-name');
    var pro_avilable = $(this).attr('data-avilable');
    var pro_id = $(this).attr('data-pro-id');
    var left = $('.gift_sets_bar .main_product_list').width() + 128;
    var pro_index = $('.main_product_list .main_product_item').length + 1;
    var total_prce = 0;
    var personalise_html = '';
    var list_array = [];

    if ($(this).parents('.product-card').find('.gift_bar_dropdown').html() != undefined){
      size_var = '<div class="custom_size_variant">' + $(this).parents('.product-card').find('.gift_bar_dropdown').html() + '</div>';
    }
    
    if(personalise_name == 'true'){
      personalise_html = '<div class="personalise"><img src="https://cdn.shopify.com/s/files/1/0289/3240/7375/files/palette.png?v=1695042120"><span class="personalise_text">Personalise</span></div>'
    }
      
    
    $('.gift_sets_bar .main_product_list').append('<div class="main_product_item" data-pro-index="'+ pro_index +'" data-price="'+ current_price +'" data-avilable="'+ pro_avilable +'" data-pro-id="'+ pro_id +'"><div class="personalise_with_close_icon">'+ personalise_html +'<span class="remove_pro_items">'+ close_svg +'</span></div><div class="image"><img src="'+ current_image_url +'"></div><h4 class="pro_title">'+ current_title +'</h4>'+ size_var +'</div>')
    $('.gift_sets_bar .bar_scroll_pro').scrollLeft(left);
    
    if($('.gift_sets_bar .main_product_list .main_product_item').length > 1){
      $('.gift_sets_bar .title-with-close_icon .bar_title').text('Selected products');
      $('.gift_sets_bar .page_wrapper .action-buttons-wrap button#addAllToCart, .gift_sets_bar .page_wrapper .action-buttons-wrap button#personaliseAll').prop('disabled', false);
    }else{
      $('.gift_sets_bar .title-with-close_icon .bar_title').text('Please select at least 2 products');
      $('.gift_sets_bar .page_wrapper .action-buttons-wrap button#addAllToCart, .gift_sets_bar .page_wrapper .action-buttons-wrap button#personaliseAll').prop('disabled', true);
    }

    $('.gift_sets_bar .main_product_list .main_product_item').each(function(e) {
      total_prce = total_prce + parseInt($(this).attr('data-price'))
      
      var size_var_array = [];
      if($(this).find('.custom_size_variant').html() != undefined){
        $(this).find('.custom_size_variant select option').each(function(e) {
          size_var_array.push($(this).attr('value')+':'+$(this).text());
        });
      }

      var personalise_name_check = false;
      if($(this).find('.personalise').length){
        personalise_name_check = true;
      }

      list_array.push({image: $(this).find('.image img').attr('src'), price: $(this).attr('data-price'), title: $(this).find('.pro_title').text(), personalise_name_check: personalise_name_check, personalise_name: $(this).find('.personalise .personalise_text').text(), product_id: $(this).attr('data-pro-id'), pro_avilable: $(this).attr('data-avilable'), size_variant: size_var_array})
    });
    $('.gift_sets_bar #setTotal').html(theme.Currency.formatMoney(total_prce,theme.moneyFormatWithCurrency))

    // Set Cookie
    var gift_bar_html = JSON.stringify(list_array);
    setCookie("Gift Sets", gift_bar_html, 30);

    if($('.gift_sets_bar.is_down').length){
      $('.gift_sets_bar .dubble_arrows').trigger('click');
    }
    
    setTimeout(function(){$('.grid-view-item .circle_check_icon').removeClass('is_added')}, 900);
  }
});

// gidt sets remove
$(document).on('click', '.gift_sets_bar .remove_pro_items', function(e){
  $(this).parents('.main_product_item').remove();
  if($('.gift_sets_bar .main_product_list .main_product_item').length < 2){
    $('.gift_sets_bar .title-with-close_icon .bar_title').text('Please select at least 2 products');
    $('.gift_sets_bar .page_wrapper .action-buttons-wrap button#addAllToCart, .gift_sets_bar .page_wrapper .action-buttons-wrap button#personaliseAll').prop('disabled', true);
  }

  var total_set_prce = 0;
  $('.gift_sets_bar .main_product_list .main_product_item').each(function(e) {
    total_set_prce = total_set_prce + parseInt($(this).attr('data-price'))
  })
  $('.gift_sets_bar #setTotal').html(theme.Currency.formatMoney(total_set_prce,theme.moneyFormatWithCurrency))

  var new_list_array = [];
  $('.gift_sets_bar .main_product_list .main_product_item').each(function(e, value) {
    var personalise_name_check = false;
    if($(this).find('.personalise').length){
      personalise_name_check = true;
    }
    ...
    var size_var_array = [];
    if($(value).find('.custom_size_variant').html() != undefined){
      $(value).find('.custom_size_variant select option').each(function(e) {
        size_var_array.push($(this).attr('value')+':'+$(this).text());
      });
    }
    
    new_list_array.push({image: $(this).find('.image img').attr('src'), price: $(this).attr('data-price'), title: $(this).find('.pro_title').text(), personalise_name_check: personalise_name_check, personalise_name: $(this).find('.personalise .personalise_text').text(), product_id: $(this).attr('data-pro-id'), pro_avilable: $(this).attr('data-avilable'), size_variant: size_var_array})
  });

  // Set Cookie
  var new_gift_bar_html = JSON.stringify(new_list_array);
  setCookie("Gift Sets", new_gift_bar_html, 30);
});

// Clear all gift sets product
$(document).on('click', '.gift_sets_bar .page_wrapper .action-buttons-wrap #startOver', function(){
  if($('.gift_sets_bar .main_product_list .main_product_item').length){
    $('.gift_sets_bar .main_product_list .main_product_item').remove();
  }
  $('.gift_sets_bar').removeClass('is_show');
});

// Close gift sets bar
$(document).on('click', '.gift_sets_bar .bar_header .bar_close_icon', function(){
  $('.gift_sets_bar').removeClass('is_show');
});

// Product personalise popup open
$(document).on('click', '.gift_sets_bar .main_product_item .personalise', function(){
  if($('.personalise_text_popup').length){
    
    var current_pro_index = $(this).parents('.main_product_item').attr('data-pro-index');
    $('.personalise_text_popup').attr('data-curr-index', current_pro_index);

    var current_prs_text = $(this).find('.personalise_text').text();
    if(current_prs_text != 'Personalise'){
      $('.personalise_text_popup input[type="text"]').val(current_prs_text);
    }
    
    $('.personalise_text_popup').addClass('is_open');
    
  }
});

// personalise popup close
$(document).on('click', '.personalise_text_popup .popup_close', function(){
  $('.personalise_text_popup').removeClass('is_open');
  if($('.personalise_text_popup.replace_all_name').length){
    $('.personalise_text_popup').removeClass('replace_all_name');
  }
});

// personalise popup out side click close
$(document).on("click", function(e) {
  if($('.personalise_text_popup.is_open').length){
    if($(e.target).parents('.popup_wrapper').length == 0 && $(e.target).parents(".personalise").length == 0 && !$(e.target).hasClass("popup_wrapper") && !$(e.target).hasClass("personaliseAll")){
      $('.personalise_text_popup').removeClass('is_open');
      $('.personalise_text_popup input[type="text"]').val('');
      if($('.personalise_text_popup.replace_all_name').length){
        $('.personalise_text_popup').removeClass('replace_all_name');
      }
    }
...
// personalise popup conform button
$(document).on('click', '.personalise_text_popup .personalise_button', function(){
  var current_pro_box_index = $(this).parents('.personalise_text_popup').attr('data-curr-index');
  var text_box = $(this).parents('.popup_wrapper').find('input[type="text"]').val();

  if(text_box.trim() != ''){
    if($(this).parents('.personalise_text_popup.replace_all_name').length){
      $('.gift_sets_bar .personalise .personalise_text').text(text_box);
    }else{
      $('.gift_sets_bar .main_product_item[data-pro-index="'+ current_pro_box_index +'"] .personalise .personalise_text').text(text_box);
    }
...
// change personalise_cntaint input text
$(document).on('input', '.personalise_cntaint input', function(){
...
// Gift bar all product text change popup
$(document).on('click', '#personaliseAll', function(){
  $('.personalise_text_popup').addClass('is_open replace_all_name');
});

// Gift bar checkbox checked
$(document).on('change', '#gift-checkbox', function(){
  if($(this).is(':checked') == true){
    $('.gift_sets_bar .select_gift .gift_list').removeClass('hide');
  }else{
    $('.gift_sets_bar .select_gift .gift_list').addClass('hide');
  }
});
...
$(document).on('click', '#addAllToCart', function(){
  var addtocart_item = []
  if($('.gift_sets_bar .main_product_list .main_product_item').length){
    $('#addAllToCart .button_text').addClass('hide');
    $('#addAllToCart .adding').removeClass('hide');
    $('.gift_sets_bar .main_product_list .main_product_item').each(function(e) {
      if($(this).attr('data-avilable') == 'true'){
        if($(this).find('.personalise .personalise_text').text() != 'Personalise'){
          if($('.gift_sets_bar .select_gift .gift-checkbox:checked').length){
            addtocart_item.push({id: $(this).attr('data-pro-id'), quantity: 1, properties: {'Name': $(this).find('.personalise .personalise_text').text(), 'Gift wrap': $('.gift_sets_bar .select_gift .gift_list .gift_select_box').val()}})
          }else{
            addtocart_item.push({id: $(this).attr('data-pro-id'), quantity: 1, properties: {'Name': $(this).find('.personalise .personalise_text').text()}})
          }
        }else{
          if($('.gift_sets_bar .select_gift .gift-checkbox:checked').length){
            addtocart_item.push({id: $(this).attr('data-pro-id'), quantity: 1, properties: {'Gift wrap': $('.gift_sets_bar .select_gift .gift_list .gift_select_box').val()}})
          }else{
            addtocart_item.push({id: $(this).attr('data-pro-id'), quantity: 1})
          }
        }
      }
    })
    ...
    if($('.gift_sets_bar .select_gift .gift-checkbox:checked').length){
      var gift_select_box = $('.gift_sets_bar .gift_select_box').val();
      var get_pro_id = $('.gift_sets_bar .gift_select_box option[value="'+ gift_select_box +'"]').attr('data-id');
      addtocart_item.push({id: get_pro_id, quantity: 1})
    }
...
$(document).on('click', '.gift_sets_bar .page_wrapper .bar_header .dubble_arrows, .gift_sets_bar .page_wrapper .bar_header .bar_title', function(){
  if($(this).parents('.gift_sets_bar').hasClass('is_down')){
    $(this).parents('.gift_sets_bar').removeClass('is_down');
  }else{
    $(this).parents('.gift_sets_bar').addClass('is_down');
  }
});

// Gift product variant change
$(document).on('click', '.gift_sets_bar .main_product_list .main_product_item .size_variant', function() {
  var cuttent_var_selected = $(this).val();
  $(this).parents('.main_product_item').attr('data-pro-id', cuttent_var_selected);
...
// CD Shubham
 function toggleButtonState() {
    const $checkbox = $('#pro_confirm_check');
    const $button = $('.pro_confirm ~ .product-form__controls-group button,.pro_confirm ~ .tab_list button');
  ...
  // Event listener for the div click
  $('.pro_confirm').on('click', function(e) {
    if (e.target.id !== 'pro_confirm_check') {
      const $checkbox = $('#pro_confirm_check');
  ...
  // Prevent label default behavior to avoid double click
  $('.pro_confirm label').on('click', function(event) {
    event.preventDefault();
  });
  ...
  // Event listener for the checkbox click to toggle state directly
  $('#pro_confirm_check').on('click', function() {
    toggleButtonState();
  });